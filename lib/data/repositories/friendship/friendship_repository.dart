...
abstract interface class FriendshipRepository {
  ...
  Future<Result<List<User>>> getFriends(String currentUserId);

  /// 현재 사용자가 받은 친구 요청 목록을 조회한다. (나 탭 > 친구)
  Future<Result<List<Friendship>>> getReceivedFriendRequests(
  ...
  Future<Result<List<User>>> getMutualFriends({
    required String currentUserId,
    required String targetUserId,
  });

  /// 추천 친구 목록을 조회한다. (사용자 검색)
  Future<Result<List<User>>> getRecommendedFriends(
    String currentUserId, {
    int limit = 10,
  });
  ...
  /// 받은 친구 요청을 수락한다.
  Future<Result<Friendship>> acceptFriendRequest(String friendshipId);

  /// 받은 친구 요청을 거절한다.
  Future<Result<void>> declineFriendRequest(String friendshipId);

  /// 보낸 친구 요청을 취소한다.
  Future<Result<void>> cancelFriendRequest(String friendshipId);
  ...
  Future<Result<List<Block>>> getBlockedUsers(String currentUserId);
